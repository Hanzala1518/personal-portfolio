const siteConfig = {
  name: "Hanzala Saify - Portfolio",
  shortName: "Hanzala's Portfolio",
  description: "Professional portfolio showcasing my skills, certifications, and projects in cybersecurity, data science, and software engineering.",
  url: "https://yourportfolio.com", // Replace with your actual domain
  keywords: [
    "portfolio",
    "cybersecurity",
    "data science",
    "software engineer",
    "web developer",
    "projects"
  ],
  author: {
    name: "Hanzala Saify", // Replace with your name
    handle: "@HanzalaSaify", // Replace with your Twitter/X handle
    email: "hanzalasaify103@gmail.com", // Replace with your email
    social: [
      "https://www.linkedin.com/in/hanzala-saify-11aa3a262/", // Replace with your LinkedIn
      "https://github.com/Hanzala1518", // Replace with your GitHub
      "https://x.com/HanzalaSaify" // Replace with your Twitter/X
    ]
  },
  nav: [
    { label: "Home", href: "/" },
    { label: "Skills", href: "/#skills" },
    { label: "Certifications", href: "/#certifications" },
    { label: "Projects", href: "/#projects" },
    { label: "Testimonials", href: "/#testimonials" },
    { label: "Contact", href: "/#contact" }
  ]
}

export default siteConfig
